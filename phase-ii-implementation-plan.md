...
## 1. Design System
  ...
  [ ] Create `css/design-system.css` with spacing scale (`--space-1` … `--space-5`) and fluid type scale using `clamp()`.
  [ ] Define core color tokens (`--brand`, `--brand-light`, `--surface`, `--surface-alt`, `--text`, `--text-muted`) and verify WCAG AA contrast.
  [ ] Import Google Fonts **Poppins** (headings) and **Inter** (body) via `font-display: swap`.

## 2. Layout Refactor

  [ ] Replace hero and facet list flex layouts with CSS Grid (`grid-template-columns: repeat(auto-fit,minmax(10rem,1fr));`).
  [ ] Convert each `.facet-section` to a named‑areas grid for balanced text / media alignment.
  [ ] Update media query breakpoints to rely on fluid grid where possible.

## 3. Navigation & Header

  [ ] Implement sticky header that shrinks on scroll (`window.scrollY > 60` adds `.shrink`).
  [ ] Add smooth‑scroll anchor links and highlight active facet via Intersection Observer.
  [ ] Set `scroll-margin-top` on section headings to avoid header overlap.
